...
from data_readers import list_sim_summary_files, get_material_str_clean, get_baseline_n
...
def create_simulation_tab(json_combined_path, summary_dir, exp_data_dir):
    ...
    sim_controls = widgets.VBox(
        [
            ...
            mode_selection, custom_modes_box, layer_selector, delta_n_widget,
            verbose_toggle
        ],
        layout=widgets.Layout(
    ...
    def on_sim_run_clicked(b):
        with sim_output:
            ...
            # Choix des layers et Δn
            selected_layers = list(layer_selector.value)
            delta_n         = delta_n_widget.value
            ...
            for idx, cfg in enumerate(sim_config_selector.value):
                ...
                # On récupère l'indice de base n0 depuis le config ou en paramètre utilisateur
                n0 = cfg.get('material', {}).get('n_ext', 1.33)
                ...
                # 3) Sensibilité spectrale Sλ (nm/RIU)
                # Pour chaque couche sélectionnée, on récupère son indice de base n₀
                # get_baseline_n parcourt cfg['material']['MATERIALS_CONFIG'] et renvoie n₀
                n0s = {
                    layer: get_baseline_n(cfg, layer, lam_dip, json_combined_path)   # clé de couche → n₀ extrait
                    for layer in selected_layers             # on boucle sur toutes les couches choisies
                }

                # On fait une copie profonde de la configuration complète
                # pour ne pas modifier l’objet cfg original utilisé ailleurs
                cfg2 = deepcopy(cfg)

                # On parcourt chaque entrée de la liste MATERIALS_CONFIG dans cfg2
                for entry in cfg2['material']['MATERIALS_CONFIG']:
                    # Si cette entrée correspond à une couche qu'on veut perturber
                    if entry['key'] in selected_layers:
                        # On force le type à 'Custom' pour écraser la loi d’indice existante
                        entry['material']['type'] = 'Custom'
                        # On définit l'expression n² avec n = n₀ + Δn
                        entry['material']['expression'] = f"({n0s[entry['key']] + delta_n})**2"

                # On relance la simulation RCWA avec la configuration modifiée (n₀+Δn)
                R1, _, _ = run_simulation_one_combo(
                    lam_range,    # tableau des longueurs d'onde
                    wave,         # paramètres d’onde (angle, polarisation,…)
                    n_modes,      # nombre de modes RCWA
                    cfg2,         # configuration modifiée avec Δn sur les layers sélectionnés
                    json_combined_path
                )
                # On convertit la liste en numpy array pour pouvoir l’interpoler
                R1 = np.array(R1)

                # 4a) Déplacement de résonance Sλ (nm/RIU)
                # find_best_dip_fwhm retourne un tuple de 26 éléments :
                # (lam_left, lam_right, fwhm, lam_dip, R_dip, …, R_syms)
                result_mod = find_best_dip_fwhm(
                    lam, R1,
                    smooth_win=0,    # pas de lissage
                    polyorder=0,     # inutile si smooth_win=0
                    dip_prom=0.01,   # proéminence minimale pour détecter le dip
                    dip_dist=5,      # distance minimale entre dips
                    peak_dist=5,     # distance minimale entre maxima
                    verbose=True     # pour tracer les marqueurs si souhaité
                )
                # Le 4ᵉ élément (index 3) du tuple est lam_dip, i.e. λ_res pour n₀+Δn
                lam_res_1 = result_mod[3]

                # Sensibilité spectrale : pente de λ_res(n) entre n₀ et n₀+Δn
                S_lambda = (lam_res_1 - lam_dip) / delta_n
                # en nm de décalage par unité d’indice (nm/RIU)

                # 4b) Sensibilité en réflectance dR/dn au pic
                # On construit une fonction continue R1(λ) par interpolation cubique
                interp_R1   = interp1d(lam, R1, kind='cubic')
                
                # On évalue cette fonction au λ_res initial (lam_dip)
                R1_at_res0  = interp_R1(lam_dip)

                # Calcul de la variation de réflectance normalisée par Δn
                S_R = (R1_at_res0 - Rdip) / delta_n
                # en unité de réflectance par unité d’indice (1/RIU)


                # 4c) Variation de R au half-level du côté de pente la plus forte
                #   on utilise lam_left et lam_right (points à mi-hauteur)
                eps = lam[1] - lam[0]  # pas spectral
                refl0 = interp1d(lam, Rup, kind='cubic')
                
                # pente autour de lam_left
                slope_left = (refl0(lam_left + eps) - refl0(lam_left - eps)) / (2 * eps)
                
                # pente autour de lam_right
                slope_right = (refl0(lam_right + eps) - refl0(lam_right - eps)) / (2 * eps)
                
                # on choisit le demi-point où la pente est la plus forte
                if abs(slope_left) > abs(slope_right):
                    half_point = lam_left
                else:
                    half_point = lam_right
                    
                # valeurs de R avant et après
                R0_half = refl0(half_point)
                refl1 = interp1d(lam, R1, kind='cubic')
                R1_half = refl1(half_point)
                delta_R_half = R1_half - R0_half

                # 5) Stockage pour le tableau
                S_lambda_summaries.append(f"{S_lambda:.1f}")
                S_dn_summaries.append(f"{S_R:.3f}")
                delta_R_half_summaries.append(f"{delta_R_half:.3f}")
                ...
                if verbose:
                     ...
                     # tracé du spectre à n0 + Δn en tirets
                    ax_plot.plot(
                        lam, R1,
                        linestyle='--',
                        color=color,
                        alpha=0.7,
                        label='_nolegend_'  # on évite de surcharger la légende
                    )
            ...
            if config_labels:
                ...
                table = ax_table.table(
                    ...
                    rowLabels=[
                        "Geometry", "Material",
                        "FWHM", r"$\lambda_0$",
                        "S_lam abs", "Sλ (nm/RIU)", r"$\mathrm{d}R/\mathrm{d}n$ (1/RIU)",
                        r"$\Delta R_{\mathrm{half}}$ (unités R)", 
                        "Q-factor", "Score dips"
                    ],
