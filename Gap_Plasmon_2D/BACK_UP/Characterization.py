...
def find_best_dip_fwhm(wavelength, reflectance,
    ...
    for j, i0 in enumerate(dips):
        ...
        # on prend ici 1% de la pente max comme seuil de “quasi-plat”
        plat_thr = 0.01 * np.nanmax(np.abs(dR_left)) if dR_left.size else 0
        ...
        # on prend ici 1% de la pente max comme seuil de “quasi-plat”
        plat_thr = 0.01 * np.nanmax(np.abs(dR_right)) if dR_right.size else 0
