...
M = list_metal.size
N = list_wavelength.size

# 1) Préparer la liste des colonnes à empiler
cols = [list_wavelength]                       # colonne 0 = λ
cols += [Ru_ito[i]   for i in range(M)]         # colonnes 1…M = R_up
cols += [Rd_ito[i]   for i in range(M)]         # colonnes M+1…2M = R_down

# 2) Empiler en un seul array 2D (N lignes, 1+2M colonnes)
table = np.column_stack(cols)  # shape (N, 1+2M)

# 3) Générer un header lisible
hdr = ["lambda_nm"]
hdr += [f"Rup_{int(t)}nm"   for t in list_metal]
hdr += [f"Rdown_{int(t)}nm" for t in list_metal]
header = " ".join(hdr)

# 4) Sauvegarder
np.savetxt(
    "R_all_thicknesses.txt",
    table,
    fmt="%.6f",
    header=header,
    comments=""
...
print(f"R_all_thicknesses.txt (shape {table.shape})")