...
angle = 0
polarization = 1           # pour TM (sinon 0 pour TE)
...
# Préparation des tableaux pour stocker les résultats (1D, car l'épaisseur est fixe)
Ru_ito = np.empty(list_wavelength.shape)
Rd_ito = np.empty(list_wavelength.shape)

# Construction de la géométrie avec l'épaisseur fixe pour la couche métallique
geometry = {
    ...
    "thick_mol": thick_mol,
    "thick_metalliclayer": thick_metal,  # épaisseur fixe
    "thick_sub": thick_sub,
...
for idx_wav, wavelength in enumerate(list_wavelength):
    ...
    Ru_ito[idx_wav], Rd_ito[idx_wav] = reflectance(geometry, wave, materials, n_mod)

# Tracé du spectre simulé (Rup)
import matplotlib.pyplot as plt
