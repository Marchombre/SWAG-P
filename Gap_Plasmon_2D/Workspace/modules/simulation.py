...
class SimulationTab:
    ...
    def __init__(self):
        ...
        self.sim_files_dropdown = widgets.Dropdown(
            ...
            style={'description_width': 'initial'})
        self.sim_refresh_button = widgets.Button(
        ...
        self.sim_refresh_button.on_click(
            lambda *_: self.sim_files_dropdown.set_trait(
                "options", list_sim_summary_files(summary_dir)))
    ...
    #                            fonction de coût                       #
    # ----------------------------------------------------------------- #
    def cost(self, x, mode="dip"):
        """
        1 – S_R pour vecteur x, en utilisant
        self.sim_lambda_min, self.sim_n_points, etc.
        """
        # 1) config cochée
        selected = [
            c for c in self.all_configs
            if self.config_checkboxes[c["config_name"]].value
        ]
        if not selected:
            raise RuntimeError("Cochez au moins une configuration pour l'optimisation.")
        cfg0 = selected[0]
        cfg  = deepcopy(cfg0)

        # 2) injecter x
        for i, (key, _) in enumerate(ordered_params):
            cfg["geometry"]["geometry"][key] = float(x[i])
        ...
        # 5) détection dip
        (dip_idxs, lam_ds, R_ds, _, lam_ls, lam_rs, *_), _ = \
            _find_dip_core(
                wavelength=lam,
                reflectance=Rup0,
                smooth_win=0, polyorder=0,
        ...
        # 6) calcule S_R
        *_, S_lambda, S_R, _ = simulate_delta_spectrum(
            cfg                = cfg,
            lam                = lam,
            wave               = wave,
            n_modes            = n_modes,
            sel_layers         = list(self.layer_selector.value),
            delta_n            = self.delta_n_widget.value,
            lam_dip            = lam_d,
            R_dip              = R_d,
            lam_left           = lam_l,
            lam_right          = lam_r,
