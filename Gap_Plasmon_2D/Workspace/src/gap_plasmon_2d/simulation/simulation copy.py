...
def _simulate_worker(args):
    ...
    (cfg, lam_range, wave, n_modes, sel_layers, delta_n,
     mode_calc, lambda0, flags, json_combined_path,
     chunk_idx, n_sub) = args
    ...
    # renvoyer aussi l’indice de chunk pour reconstituer le spectre
    return dict(
        cfg_name  = name,
        cfg       = cfg,
        Rup       = Rup,
...
class SimulationTab:
    ...
    def __init__(self):
        ...
        # 3) Création de la figure & de l’axe de tracé
        ...
        #         overflow_x='hidden',
        #         min_width='0',
        #         margin='10px 0 0 0'    # ← 20px d’espace au-dessus
        #     )
        # )
    ...
    def _init_common_widgets(self):
        ...
        self.custom_modes_box = widgets.VBox(
            ...
            layout=widgets.Layout(width='auto', 
                                  height='150px',  # ajuste à la hauteur souhaitée
                                  overflow_y='auto',  # barre de scroll interne
                                  border='1px solid lightgray',
                                  padding='2px' 
                                  )
            )
        
        # on cache d’emblée la zone custom
        self.custom_modes_box.layout.display = 'none'

        # 1) quand on change de mode RCWA → show/hide + rebuild
        self.mode_selection.observe(self._on_toggle_custom, names='value')
        ...
        for cfg in self.all_configs:
            ...
            chk = widgets.Checkbox(
                ...
                description=name,
                indent=False,
                layout=Layout(width='150px', height='20px'),
                style={'description_width':'initial'}
            )            
            
            dn  = widgets.Checkbox(value=False, description='Δn', indent=False,
                    layout=Layout(width='50px', height='20px'),
                    style={'description_width':'initial'})
            
            chk.observe(self._update_sim_run_button, names='value')
            # 2) quand on (dé)coche une config → si on est en custom, on rebuild
            chk.observe(self._on_toggle_custom, names='value')
        ...
        max_rows = 10                       # par exemple on ne veut jamais plus de 10 lignes visibles
        row_height = 30                     # hauteur approximative d’une ligne
        header_height = 40                  # pour la ligne de boutons « Tout sélectionner »
        total_h = header_height + row_height*max_rows
        ...
        self.config_list = widgets.VBox(
            ...
            *rows],
            layout=widgets.Layout(
                width='500px',               # largeur fixe, comme dans optimisation
                height=f'{total_h}px',       # hauteur fixe, dépassement scrollable
                overflow_y='auto',           # active le scroll interne
                border='1px solid lightgray',
                padding='5px',
                display='none',
                flex='0 0 auto'              # empêche l’expansion en flex
            )
        )
    ...
    def _build_panels(self):
        ...
        self.panel_controls = widgets.VBox(
            [
                ...
                self.runtime_box,
                self.verbose_toggle,
                # → Déplacement de la sélection de fichiers sous le verbose
                widgets.VBox(
                    [self.sim_files_dropdown, self.sim_download_button],
                    layout=widgets.Layout(gap='10px')
                )
    ...
    def _on_run(self, _):
        ...
        # ----------------------------------------------------------------
        # 3) Fractionnement en n_sub tâches pour suivi progressif par chunk
        # ----------------------------------------------------------------
        n_sub = 10
        lam_chunks = np.array_split(lam_range, n_sub)
        # on stocke pour tracer chaque chunk dès réception
        self._lam_chunks = lam_chunks
        self._cfg_order  = [cfg['config_name'] for cfg in selected_cfgs]

        args_list = []
        for cfg in selected_cfgs:
            n_modes = self._get_n_modes_for(cfg['config_name'])
            for chunk_idx, lam_chunk in enumerate(lam_chunks):
                args_list.append((
                    cfg, lam_chunk, wave, n_modes,
                    sel_layers, delta_n,
    ...
    def _simulate_many(self, *, args_list, progress_queue):
        ...
        try:
            ...
            for i, out in enumerate(pool.imap_unordered(_simulate_worker,
                ...
                # 1) émettre immédiatement chaque chunk
                if progress_queue is not None:
                    progress_queue.put(("CHUNK", out))
                # 2) puis progression globale
                results.append(out)
                if progress_queue is not None:
    ...
    def _check_process(self):
        ...
        # ─── on reschedule d’emblée pour ne pas bloquer la boucle ───
        loop = get_ipython().kernel.io_loop
        loop.add_timeout(loop.time() + 0.1, self._check_process)
        ...
        if tag == "CHUNK":
            chunk = payload[0]
            self._on_new_chunk(chunk)
            # on continue à dépouiller la queue sans rescheduler tout de suite


        # ─── 1) mise à jour PROG → mise à jour de la barre
        elif tag == "PROG":
            frac = payload[0]                          # valeur 0–1
            # on efface tout message texte précédent (ex. “Done”)
            self._status_html.value       = ""
        ...
        # ----------------------------------------------------------------
            ...
            # on force un draw ultra-rapide du canvas vide pour
            # libérer l’UI AVANT de lancer le post-traitement lourd  
            with self.canvas_output:
                clear_output(wait=True)
                display(self.fig.canvas)

            # On reporte _build_outputs dans l’event-loop,
            #  0ms plus tard, pour laisser le navigateur
            #  respirer et afficher le canvas…
            loop.add_callback(lambda: self._build_outputs(payload[0]))
            
            return  # on ne reprogrammera pas un autre tick
    ...
    # ----------------------------------------------------------------- #
    #  Traceur incrémental de chaque chunk reçu                         #
    # ----------------------------------------------------------------- #
    def _on_new_chunk(self, chunk):
        """
        Trace immédiatement le sous-spectre reçu.
        chunk = {'cfg_name','Rup','chunk_idx','n_sub',...}
        """
        name      = chunk['cfg_name']
        idx_cfg   = self._cfg_order.index(name)
        color     = plt.rcParams['axes.prop_cycle'].by_key()['color'][idx_cfg % len(self._cfg_order)]
        lam_chunk = self._lam_chunks[chunk['chunk_idx']]
        Rup       = chunk['Rup']

        with self.canvas_output:
            # tracé semi-transparent pour voir le remplissage progressif
            self.ax_plot.plot(lam_chunk, Rup,
    ...
    def _build_outputs(self, results):
        ...
        # 1) avant toute action lourde, on s’assure que le canvas a un update
        with self.canvas_output:
            self.ax_plot.set_title("Rendering…")  
        ...
        # Reconstruit uniquement l’axe de tracé
        self.fig.clf()
        self.ax_plot = self.fig.add_subplot(1, 1, 1)
        self.ax_plot.clear()
        # self.ax_plot.set_xlabel("λ (nm)")
        # self.ax_plot.set_ylabel("Reflectance")
        self.fig.subplots_adjust(left=0.15, right=0.98, top=0.90, bottom=0.10)
        ...
        try:
            ...
            # ─── Regrouper par config et reconstituer chaque spectre ---------
            from collections import defaultdict
            grouped = defaultdict(list)
            for res in results:
                grouped[res['cfg_name']].append(res)

            reconstructed = []
            
            for name, res_list in grouped.items():
                # trier par chunk_idx et concaténer
                res_list.sort(key=lambda r: r['chunk_idx'])
                full_Rup = np.concatenate([r['Rup'] for r in res_list])
                # prendre details du 1er chunk
                details = res_list[0]['details']
                cfg     = res_list[0]['cfg']
            ...
            # ─── Maintenant on SAUVE chacun des spectres complets -------------
            # on le fait _avant_ le tracé pour être sûr que summary_sim_dir
            # contient le JSON correspondant.
            for entry in reconstructed:
                ...
                try:
                    save_simulation_summary(
                        ...
                        lam_range,
                        wave,
                        self._get_n_modes_for(nm),
                        summary_sim_dir,
                        custom_name=nm,
                        fwhm_summaries   = [ float(det.get('fwhm', np.nan)) ],
                        lam_summaries    = [ float(det.get('lambda0', np.nan)) ],
                        delta_lam_over_midLam_summaries = [
                            det.get('extra_metrics', {}).get('Δλ/λₘᵢₙ', "")
                        ],
                        Q_factor         = [ float(det.get('extra_metrics', {}).get('Q-factor', np.nan)) ],
                        best_S_R         = [ float(det.get('extra_metrics', {}).get('ΔR/Δn', np.nan)) ]
                    )
                except ValueError as e:
                    # on ignore le mismatch et on continue, pour ne pas bloquer le plot
                    logger.warning(f"save_simulation_summary ignoré pour '{nm}': {e}")

            # ─── On remplace maintenant 'results' par la liste reconstituée ----
            # remplacer results par la liste reconstituée
            results = reconstructed

            # ─── Maintenant boucle classique sur results complets ----------
                
            
            for idx, res in enumerate(results):
                cfg      = res['cfg']
                name     = res['cfg_name']
                Rup_base = np.asarray(res['Rup'], dtype=float)
                details  = res['details']
            ...
            with self.canvas_output:
                ...
                self.fig.canvas.draw()
                display(self.fig.canvas)
                display(_download_link(self.fig, f"simulation_{datetime.now():%Y%m%d_%H%M%S}.png"))
            ...
            # ─── Construction du tableau matplotlib dans une figure dédiée ─
            # Préparation des cellText, colLabels, rowLabels (identique à l’existant)
            # fig_table = plt.figure(figsize=(8, 3))
            # ax_tab    = fig_table.add_axes([0.10, 0.05, 0.85, 0.90])
            # ax_tab.axis('off')
            ...
            # # applique font, couleurs, hauteurs comme avant…
            ...
            # # ———> on encapsule fig_table dans un canvas interactif
            # from matplotlib.backends.backend_nbagg import FigureCanvasNbAgg

            # # 1) encapsulation de la figure brute dans un canvas interactif
            # table_canvas = FigureCanvasNbAgg(fig_table)
            # # on ne masque pas le canvas : on laisse display par défaut
            # table_canvas.layout = widgets.Layout(width='100%', min_width='0')

            # # 2) Dans la sortie dédiée, on clear, on draw puis on display
            # with self.table_output:
            #     clear_output(wait=True)
            #     table_canvas.draw()               # ← déclenche tout de suite le rendu
            #     display(table_canvas)             # ← on affiche le canvas dessiné
            #     display(_download_link(
            #         fig_table,
            #         f"tableau_{datetime.now():%Y%m%d_%H%M%S}.png"
            #     ))

            # # 3) on ferme la figure brute pour éviter les fuites mémoire
            # plt.close(fig_table)
    ...
    def _on_toggle_custom(self, change):
        """
        Show/hide et rebuild de self.custom_modes_box
        lorsque l’on change de mode RCWA ou (dé)coche une config.
        """
        # affichage uniquement en mode custom
        if self.mode_selection.value == 'custom':
            self.custom_modes_box.layout.display = ''
            self._refresh_custom_modes()
        else:
            self.custom_modes_box.layout.display = 'none'
