...
lib_path = os.path.join(db_path, paths.CATALOG_NK)
...
class NKExplorerWidget:
    ...
    def update_data(self):
        ...
        for i in range(len(page_ids)):
            if page_ids[i] == "":
                ...
                k.append(0)
                continue
            data_path = os.path.join(db_path, str(paths.DATA_DIR), page_paths[i])
    ...
    def update_details(self):
        ...
        if selected_index is not None:
            data_path = os.path.join(db_path, str(paths.DATA_DIR), page_paths[selected_index])
            data_path = os.path.normpath(data_path)
            if os.path.exists(data_path):
                with open(data_path, "r", encoding="utf-8") as file:
                    datafile = yaml.safe_load(file)
    ...
    def update_about(self):
        ...
        if selected_index is not None:
            ...
            about_path1 = os.path.join(db_path, str(paths.DATA_DIR), dir_1up, "about.yml")
            about_path2 = os.path.join(db_path, str(paths.DATA_DIR), dir_2up, "about.yml")
            about1 = ''
            names1 = []
            links1 = []
            about2 = ''
